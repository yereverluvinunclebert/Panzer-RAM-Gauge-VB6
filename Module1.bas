...
Public Sub mnuSupport_ClickEvent()
    ...
    If answer = vbYes Then
        Call ShellExecute(menuForm.hwnd, "Open", "https://github.com/yereverluvinunclebert/Panzer-RAM-Gauge-VB6/issues", vbNullString, App.path, 1)
    End If
...
Public Sub readPrefsPosition()
   ...
   On Error GoTo readPrefsPosition_Error
    ...
    If gblDpiAwareness = "1" Then
        gblFormHighDpiXPosTwips = fGetINISetting("Software\PzRAMGauge", "formHighDpiXPosTwips", gblSettingsFile)
        gblFormHighDpiYPosTwips = fGetINISetting("Software\PzRAMGauge", "formHighDpiYPosTwips", gblSettingsFile)
...
'        End If
    Else
        gblFormLowDpiXPosTwips = fGetINISetting("Software\PzRAMGauge", "formLowDpiXPosTwips", gblSettingsFile)
        gblFormLowDpiYPosTwips = fGetINISetting("Software\PzRAMGauge", "formLowDpiYPosTwips", gblSettingsFile)
   ...
   On Error GoTo writePrefsPosition_Error
    ...
    If panzerPrefs.WindowState = vbNormal Then ' when vbMinimised the value = -48000  !
        If gblDpiAwareness = "1" Then
            ...
            sPutINISetting "Software\PzRAMGauge", "formHighDpiXPosTwips", gblFormHighDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzRAMGauge", "formHighDpiYPosTwips", gblFormHighDpiYPosTwips, gblSettingsFile
        Else
            gblFormLowDpiXPosTwips = CStr(panzerPrefs.Left)
            gblFormLowDpiYPosTwips = CStr(panzerPrefs.Top)
            
            ' now write those params to the toolSettings.ini
            sPutINISetting "Software\PzRAMGauge", "formLowDpiXPosTwips", gblFormLowDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzRAMGauge", "formLowDpiYPosTwips", gblFormLowDpiYPosTwips, gblSettingsFile
...
Public Sub hardRestart()
    ...
    If fFExists(thisCommand) Then
        ...
        Call ShellExecute(panzerPrefs.hwnd, "open", thisCommand, "Panzer RAM Gauge.exe prefs", "", 1)
    Else
