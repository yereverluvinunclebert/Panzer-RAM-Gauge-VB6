...
Public Sub mainRoutine(ByVal restart As Boolean)
    ...
    ' = "none"
    gblStartupFlg = True
    gblWidgetName = "Panzer RAM Gauge"
    thisPSDFullPath = App.Path & "\Res\Panzer RAM Gauge VB6.psd"
    ...
    menuForm.mnuAbout.Caption = "About Panzer RAM Gauge Cairo " & gblCodingEnvironment & " widget"
    ...
    Call readSettingsFile("Software\PzRAMGauge", gblSettingsFile)
...
Private Sub checkFirstTime()
   ...
   On Error GoTo checkFirstTime_Error
    ...
    If gblFirstTimeRun = "true" Then
        ...
        sPutINISetting "Software\PzRAMGauge", "firstTimeRun", gblFirstTimeRun, gblSettingsFile
    End If
...
'        gblVolumeBoost = fGetINISetting(Location, "volumeBoost", gblSettingsFile)
        ...
        ' other
        gblGaugeHighDpiXPos = fGetINISetting("Software\PzRAMGauge", "gaugeHighDpiXPos", gblSettingsFile)
        gblGaugeHighDpiYPos = fGetINISetting("Software\PzRAMGauge", "gaugeHighDpiYPos", gblSettingsFile)
        gblGaugeLowDpiXPos = fGetINISetting("Software\PzRAMGauge", "gaugeLowDpiXPos", gblSettingsFile)
        gblGaugeLowDpiYPos = fGetINISetting("Software\PzRAMGauge", "gaugeLowDpiYPos", gblSettingsFile)
...
Private Sub getToolSettingsFile()
    ...
    gblSettingsDir = fSpecialFolder(feUserAppData) & "\PzRAMGauge" ' just for this user alone
...
Private Sub handleUnhideMode(ByVal thisUnhideMode As String)
    ...
    If thisUnhideMode = "unhide" Then     'parse the command line
        gblUnhide = "true"
        sPutINISetting "Software\PzRAMGauge", "unhide", gblUnhide, gblSettingsFile
